...
Opening the popup or pressing rescan reads page text, contact-related metadata,
and links from the current page and accessible frames and open shadow roots.
Temporary scan snapshots are cleared after retrieval. Results remain in popup
...
Copying emails writes them to your system clipboard. Exporting creates a local
CSV file. Clipboard contents and exported files are outside LeadFox storage;
delete them separately when no longer needed. Opening a social profile or
source link navigates to that website, which has its own privacy practices.
